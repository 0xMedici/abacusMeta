...
contract Vault is ReentrancyGuard, ReentrancyGuard2, Initializable {
    ...
    mapping(uint256 => uint256) public payoutSize;
    ...
    */
    ...
    function initialize(
        ...
        factory = IVaultFactory(controller.factoryVersions(_vaultVersion));
        veAbc = IVeAbc(controller.veAbcToken());

        vaultVersion = uint8(_vaultVersion);
        heldToken = _heldTokenCollection;
        heldTokenId = _heldTokenIds;
    ...
    function startEmission() external {
        require(msg.sender == heldToken.ownerOf(heldTokenId));
    ...
    function purchase(
        ...
        for(uint256 i=0; i<length; i++) {
            ...
            require(ticketsPurchased[tickets[i]] + amountPerTicket[i] <= 1000e18);
            ...
            if(tickets[i] != 0 && msg.sender != address(this)) require(ticketsPurchased[tickets[i] - 1e18] == 1000e18);
            ...
            for(uint256 j = poolEpoch; j < finalEpoch; j++) {
                ...
                payoutSize[j] += amountPerTicket[i] * 0.001 ether / 1e18;
            }
    ...
    */
    function closeNft() nonReentrant external {
        uint256 poolEpoch = (block.timestamp - startTime) / 1 days;
        epochOfClosure = poolEpoch;
        require(!poolClosed);
        ...
        if(closePoolContract == address(0)) {
            IClosePool closePoolMultiDeployment = IClosePool(ClonesUpgradeable.clone(_closePoolMultiImplementation));
        ...
        heldToken.transferFrom(msg.sender, address(closePoolContract), heldTokenId);

        poolClosed = true;
        payable(msg.sender).transfer(payoutSize[poolEpoch]);
    }
    ...
    function adjustTicketInfo(
        ...
        */
        ...
        for(uint256 i=checkpoint; i<checkpoint+1; i++) {
            if(maxTicketPerEpoch[epochOfClosure] + 1e18 < _finalNftVal) {
                complete = true;
                break; 
            }
            uint256 ticketId = trader.listOfTickets[i];
            ...
            if(ticketId > _finalNftVal) {
                toRemoveNom += trader.nominalPerTicket[ticketId];
                delete trader.ticketsOwned[ticketId];
                delete trader.nominalPerTicket[ticketId];
            }
            else if(ticketId + 1e18 > _finalNftVal) {
                toRemoveNom += 
                    trader.nominalPerTicket[ticketId] * (ticketId + 1e18 - _finalNftVal) / 1e18;
                trader.ticketsOwned[ticketId] -= 
                    trader.ticketsOwned[ticketId] * (ticketId + 1e18 - _finalNftVal) / 1e18;
        ...
        for(uint256 j = epochOfClosure; j < trader.finalEpoch; j++) {
            trader.nominalTokensPerEpoch[j] -= toRemoveNom;
        }

        uint256 auctionPremium = IClosePool(closePoolContract).getAuctionPremium();
        if(!complete) closureCheckpoint[_user] += 1;
        else {
            closureCheckpoint[_user] = 0;
            IClosePool(closePoolContract).payout(
                _user, 
                auctionPremium * trader.nominalTokensPerEpoch[epochOfClosure] / totalNominalTokensPerEpoch[epochOfClosure]
            );
        }
    }
    ...
    */
    ...
    function unlockTokens(address _caller, address _user) internal {
        ...
        epochVault.updateEpoch(address(heldToken), _user, amountCreditsDesired);
